...
<Nonce><![CDATA[%(nonce)s]]></Nonce>
    ...
    def extract(self, xmltext):
        ...
        try:
            ...
            appid = xml_tree.find("AppId")
            if touser_name:
                return  ierror.WXBizMsgCrypt_OK, encrypt.text, touser_name.text
            elif appid:
                return  ierror.WXBizMsgCrypt_OK, encrypt.text, appid.text
            else:
...
class Prpcrypt(object):
    ...
    def encrypt(self,text,appid):
        ...
        @return: 加密得到的字符串
        """
        # 16位随机字符串添加到明文开头  struct.pack("I",socket.htonl(len(text)))
        # print(struct.pack("I",socket.htonl(len(text))),'============test')
        # 16位随机字符串添加到明文开头
        ...
        # # print(text, appid,'===============appid')
        # # text = self.get_random_str() + text + appid
        # # 使用自定义的填充方式对明文进行补位填充
        # pkcs7 = PKCS7Encoder()
